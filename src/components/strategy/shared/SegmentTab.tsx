...
import type { SegmentTabPerformance } from './types'

interface SegmentTabProps {
  performances: SegmentTabPerformance[]
  contextLabel?: string // '이벤트' 또는 '운영변경'
}

export function SegmentTab({ performances, contextLabel = '이벤트' }: SegmentTabProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
        ...
        <p className="text-sm text-slate-500 mb-6">
          {hasSegmentComparisonData
            ? `${contextLabel} 전후 각 세그먼트의 고객 수 변화와 유입/유출 현황입니다.`
            : segmentPeriodInfo?.comparisonSource === 'SEASONAL'
